...
public class LineListController {
	...
	public List<UIMetadataTemplate> getLineListTemplates(@RequestParam long projectId, Locale locale) {
		/*
		TODO: Not return templates - just list of names and identifiers for templates
		 */
		Project project = projectService.read(projectId);
		boolean editable = canUserEdit(project);
		List<ProjectMetadataTemplateJoin> templateJoins = metadataTemplateService.getMetadataTemplatesForProject(project);
		return templateJoins.stream().map(join -> {
			final MetadataTemplate template = join.getObject();
			List<UIMetadataField> fields = template.getFields().stream().map(field -> new UIMetadataField(field.getId(),
					field.getLabel(), field.getType(), field.getFieldKey(), editable)).collect(
					Collectors.toList());
			fields.add(0, new UIMetadataField(0L, "Name", "name", "name", editable ));
			return new UIMetadataTemplate(template.getId(), template.getLabel(), fields);
		}).collect(
				Collectors.toList());
...
//		return templates;
	...
	@ResponseBody
	public List<UIMetadataField> getAllProjectMetadataFields(@RequestParam long projectId) {
		Project project = projectService.read(projectId);
		boolean editable = canUserEdit(project);
		List<MetadataTemplateField> fields = metadataTemplateService.getMetadataFieldsForProject(project);

		List<MetadataTemplate> templates = metadataTemplateService.getMetadataTemplatesForProject(project).stream().map(
				ProjectMetadataTemplateJoin::getObject).collect(
				Collectors.toList());

		// loop and add toe fields
		...
		List<UIMetadataField> allFields =  fields.stream()
				.map(field -> new UIMetadataField(field.getId(), field.getLabel(), field.getType(), field.getFieldKey(),
						editable))
...
//	 */
//	private List<UIMetadataField> formatTemplateForUI(MetadataTemplate template, List<UIMetadataField> allFieldsAgGridColumns,
//			boolean canEdit) {
//		/*
...
//		 */
//		List<UIMetadataField> fields = new ArrayList<>();
//
//		/*
...
//			fields.add(new UIMetadataField(field, canEdit));
//		}
//
//		/*
